...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def torch_mask_tokens_dic(self, inputs: Any, dic, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        import torch
        ...
        def get_special_tokens_mask_dic(
            ...
            special_tokens_mask = [1 if token in {dic.mak_index,dic.bos_index,dic.eos_index,dic.pad_index,dic.unk_index} else 0 for token in token_ids_0]

            return special_tokens_mask

        if special_tokens_mask is None:
            special_tokens_mask = [
                get_special_tokens_mask_dic(val, already_has_special_tokens=True) for val in labels.tolist()
            ]
            special_tokens_mask = torch.tensor(special_tokens_mask, dtype=torch.bool)
        else:
            special_tokens_mask = special_tokens_mask.bool()
        ...
        # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).bool() & masked_indices
        inputs[indices_replaced] = dic.mak_index #self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)

        # 10% of the time, we replace masked input tokens with random word
        indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & masked_indices & ~indices_replaced
        random_words = torch.randint(len(dic), labels.shape, dtype=torch.long)
        if torch.cuda.is_available() and inputs.is_cuda:
            indices_random = indices_random.cuda()
